...
<fieldset>  <legend>MapCheckReportMaster</legend> 
    <form method='POST' id='dataFrom'> 
        <div class="row">
            ...
            <div class='col-lg-2 col-sm-2 col-md-2 col-xs-6'>
                ...
                <input type='text' name='buildingDescription' id='buildingDescription' class='form-control'/>
            </div>
            <div class='col-lg-2 col-sm-2 col-md-2 col-xs-6'>
                <label>Sn</label>
...
</fieldset>  <br> <fieldset>  <legend>MapCheckReportMaster Data</legend> 
    <div class='row' id='table'></div>
</fieldset>
<script>
    function edit(sn) {
        var id = ['id', 'buildingDescription', 'sn', 'type'];
...
<script>
    function doSave()
    {
        ...
        var URL = "<%=path%>/api/Utility/MapCheckReportMaster";
    ...
    function doUpdate()
    {
        ...
        var URL = "<%=path%>/api/Utility/MapCheckReportMaster/1";
...
<tr><th>Id</th><th>Building Description</th><th>Sn</th><th>Type</th><th>Action</th></tr></thead><tbody></tbody></table>`;
                var tableData;
                for (var i = 0; i < data.length; i++)
                {
                    tableData = "<tr><td>" + data[i].id + "</td><td>" + data[i].buildingDescription + "</td><td>" + data[i].sn + "</td><td>" + data[i].type + "</td><td><a onclick='edit(" + (i + 1) + ")' class='glyphicon glyphicon-edit'></a> | <a onclick='recordDelete(" + data[i].id + ")' class='glyphicon glyphicon-remove-circle'></a></td></tr>";
    ...
    function recordDelete(id) {
        if (!confirm('Are you sure'))
            return;
        var URL = "<%=path%>/api/Utility/MapCheckReportMaster/" + id;
        callApi(URL, "", "DELETE");
    }
    getRecord();
</script>
...
<%--
return "\n{\"id\": \""+id+"\",\"buildingDescription\": \""+buildingDescription+"\",\"sn\": \""+sn+"\",\"type\": \""+type+"\"}";
--%>