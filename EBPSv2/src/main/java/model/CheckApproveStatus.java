...
public class CheckApproveStatus {
	...
	public boolean get(String userType, int formId, long id) {
		 ...
		 * " COALESCE(engr_approval,'N')"; } else if (userType.equalsIgnoreCase("B")) {
		 * approveBy = " COALESCE(sub_engr_approval,'N')"; } else if
		 * (userType.equalsIgnoreCase("C")) { approveBy =
		 * " COALESCE(chief_approval,'N')"; } else if (userType.equalsIgnoreCase("D")) {
		 * approveBy = " COALESCE(designer_approval,'N')"; } else if
		 * (userType.equalsIgnoreCase("R")) { approveBy =
		 * " COALESCE(rajasow_approval,'N')"; } else if
		 * (userType.equalsIgnoreCase("AD")) { approveBy =
		 * " COALESCE(amin_approval,'N')"; } else if (userType.equalsIgnoreCase("E")) {
		 * approveBy = " COALESCE(poste_approval,'N')"; } else if
		 * (userType.equalsIgnoreCase("F")) { approveBy =
		 * " COALESCE(postf_approval,'N')"; } else if (userType.equalsIgnoreCase("G")) {
		 * approveBy = " COALESCE(postg_approval,'N')"; }
		 */

		/*
		 * String approveColumn; if (userType.equalsIgnoreCase("A")) { approveColumn =
		 * "COALESCE(er_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("B")) { approveColumn =
		 * "COALESCE(ser_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("C")) { approveColumn =
		 * "COALESCE(chief_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("R")) { approveColumn =
		 * "COALESCE(rw_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("AD")) { approveColumn =
		 * "COALESCE(amini_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("E")) { approveColumn =
		 * "COALESCE(e_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("F")) { approveColumn =
		 * "COALESCE(f_status,'P') \"approveBy\""; } else if
		 * (userType.equalsIgnoreCase("G")) { approveColumn =
		 * "COALESCE(g_status,'P') \"approveBy\""; } else { setMsg("Invalid User type");
		 * return false; }
		 */
...
//        String groupType = message.getGroupType(message.map.get("constructionType").toString());
		...
		try {
			...
			sql = "SELECT (SELECT table_name FROM ebps_tables WHERE id=table_id) AS \"previousTable\",G.form_id AS \"cFormId\",group_position,F.name \"formName\" FROM form_group G,form_name_master F,form_permissions P WHERE G.form_id=F.id AND F.id=P.form_id AND G.group_type='"
					+ groupType + "' AND G.user_type='" + userType + "' AND " + approveBy + "='Y' AND F.ID NOT IN(" + hasRevised + "," + classGroup
...
//            sql = "SELECT coalesce(status,'P') \"approveBy\" FROM status WHERE form_id='"+formId+"' AND user_type='"+userType+"' AND application_no=" + id;
			sql = "SELECT coalesce(status,'P') \"approveBy\" FROM status WHERE form_id=(select id from form_name_master WHERE table_id=(select id FROM ebps_tables where table_name='"
					+ previousTable + "')) AND user_type='" + userType + "' AND application_no=" + id;
			try {
				message.map = (Map) message.db.getRecord(sql).get(0);
				approveBy = message.map.get("approveBy").toString();
				if (approveBy.equalsIgnoreCase("A")) {
