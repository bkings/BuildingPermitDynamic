...
@Service
public class SuperStructureBuildServiceImp implements SuperStructureBuildService {
    ...
    public Object getAll(long applicationNo) {
        message.list = da.getAll("from SuperStructureBuild where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "22"));
        message.map.put("history", message.getHistory(applicationNo, "22"));
        return message.map;
    }
    ...
    public Object save(SuperStructureBuild obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "22", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 22, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "22", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "22");
    }
}