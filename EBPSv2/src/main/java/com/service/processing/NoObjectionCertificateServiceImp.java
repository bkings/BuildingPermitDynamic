...
import com.dao.processing.DaoImpNoObjectionCertificate;
import com.dao.processing.DaoNoObjectionCertificate;
import com.model.application.ApplicationApproved;
import com.model.processing.NoObjectionCertificate;
import com.model.processing.NoObjectionCertificateDetails;
import com.model.processing.NoObjectionCertificateDetailsPK;

import model.DB;

@Service
public class NoObjectionCertificateServiceImp implements NoObjectionCertificateService {

	DaoNoObjectionCertificate da = new DaoImpNoObjectionCertificate();
	model.Message message = new model.Message();
	String msg = "", sql, form = "87";
	int row;
	...
	public Object getindex(long applicationNo) {
		...
		Map map;
		List list;
		sql = "SELECT R.ENTER_BY \"enterBy\",R.ENTER_DATE \"enterDate\",R.ER_DATE \"erDate\",R.ER_NAME \"erName\",R.ER_STATUS \"erStatus\",R.SER_DATE \"serDate\",R.SER_NAME \"serName\",R.SER_STATUS \"serStatus\",R.json_data \"jsonData\" FROM building_permit_application A LEFT JOIN  no_objection_certificate R ON A.id=R.application_no WHERE A.id='"
				+ applicationNo + "'";
		...
		map = (Map) list.get(0);
		sql = "SELECT M.id AS \"descriptionId\",M.building_description description,M.SN SN,M.type,D.application_no \"applicationNo\",coalesce(D.design_data,'') AS \"designData\",coalesce(D.remark,'') AS remark FROM map_check_report_master M ,no_objection_certificate_details D WHERE M.ID=D.class_id AND D.application_no='"
				+ applicationNo + "' UNION SELECT M.id AS \"descriptionId\",M.building_description description,M.sn,M.type,'" + applicationNo
				+ "' \"applicationNo\",'' AS \"designData\",'' AS remark FROM map_check_report_master M WHERE M.ID NOT IN(SELECT D.class_id FROM no_objection_certificate_details D WHERE D.application_no='"
	...
	public Object save(Long applicationNo, String jsonData, String Authorization) {
		try {
			...
			NoObjectionCertificate obj = new NoObjectionCertificate();
			NoObjectionCertificateDetails details = new NoObjectionCertificateDetails();
			NoObjectionCertificateDetailsPK pk = new NoObjectionCertificateDetailsPK();
