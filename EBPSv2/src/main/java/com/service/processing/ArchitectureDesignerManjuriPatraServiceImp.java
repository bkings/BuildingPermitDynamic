...
import com.dao.processing.DaoArchitectureDesignerManjuriPatra;
...
@Service
public class ArchitectureDesignerManjuriPatraServiceImp implements ArchitectureDesignerManjuriPatraService {

    @Autowired
    DaoArchitectureDesignerManjuriPatra da;
    DB db = new DB();
    ...
    public Object getAll(String Authorization, String applicationNo) {
        ...
        message.list = da.getAll("from ArchitectureDesignerManjuriPatra where applicationNo='" + applicationNo + "'");
...
//            return message.respondWithError("Application no invalid or not a procee in AnusuchiGha");
        ...
        ApplicationLog.save("api/Processing/Phase1/ArchitectureDesignerManjuriPatra", "GET", td.getUserName(), "get the form");
        return message.map;
    }
    ...
    public Object save(ArchitectureDesignerManjuriPatra obj, String Authorization) {
        ...
        sql = "SELECT ER_STATUS erstatus,SER_STATUS serstatus FROM architecture_designer_manjuri_patra WHERE APPLICATION_NO='" + applicationNo + "'";
        ...
        msg = da.getMsg();
        if (row == 1) {
            ApplicationLog.save("api/Processing/Phase1/ArchitectureDesignerManjuriPatra", "POST", td.getUserName(), "save or update form");
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "71");
    }
}/*  
        JWTToken td = new JWTToken(Authorization);
        if (!td.isValid()) {
