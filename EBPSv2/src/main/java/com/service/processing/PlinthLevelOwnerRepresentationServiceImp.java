...
@Service
public class PlinthLevelOwnerRepresentationServiceImp implements PlinthLevelOwnerRepresentationService {
    ...
    public Object getAll(long applicationNo) {
        message.list = da.getAll("from  PlinthLevelOwnerRepresentation where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "23"));
        message.map.put("history", message.getHistory(applicationNo, "23"));
        return message.map;
    }
    ...
    public Object save(PlinthLevelOwnerRepresentation obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "23", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 23, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "23", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "23");
    }
}