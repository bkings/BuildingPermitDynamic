...
@Service
public class MyadThapTippaniServiceImp implements MyadThapTippaniService {
    ...
    public Object getAll(long applicationNo) {
        message.list = da.getAll("from MyadThapTippani where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "42"));
        message.map.put("history", message.getHistory(applicationNo, "42"));
        return message.map;
    }
    ...
    public Object save(MyadThapTippani obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "42", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {            
                message.db.save(message.getEnterByStatus(userType, 42, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "42", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
            return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "42");
    }
}