...
@Service
public class PuranoGharSampannaPramanPatraServiceImp implements PuranoGharSampannaPramanPatraService {
	...
	public Object getAll(Long applicationNo) {
		message.list = da.getAll("from PuranoGharSampannaPramanPatra where applicationNo=" + applicationNo);
		...
		message.map.put("comment", message.getComment("" + applicationNo, "51"));
		message.map.put("history", message.getHistory(applicationNo, "51"));
		return message.map;
	}
	...
	public Object save(PuranoGharSampannaPramanPatra obj, String Authorization) {
		...
		if (!message.checkSaveStatus(userType, "51", applicationNo)) {
		...
		try {
			row = da.save(obj);
			if (row == 1) {
				message.db.save(message.getEnterByStatus(userType, 51, applicationNo));
				message.setHistory(applicationNo, td.getUserType(), "51", "S", td.getUserName());
	...
	public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
		Object retObj = new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "51");
