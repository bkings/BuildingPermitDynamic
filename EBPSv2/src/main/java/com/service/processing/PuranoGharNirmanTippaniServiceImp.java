...
@Service
public class PuranoGharNirmanTippaniServiceImp implements PuranoGharNirmanTippaniService {
    ...
    public Object getAll(Long applicationNo) {
        message.list = da.getAll("from PuranoGharNirmanTippani where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "50"));
        message.map.put("history", message.getHistory(applicationNo, "50"));
        return message.map;
    }
    ...
    public Object save(PuranoGharNirmanTippani obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "50", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 50, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "50", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "50");
    }
}
