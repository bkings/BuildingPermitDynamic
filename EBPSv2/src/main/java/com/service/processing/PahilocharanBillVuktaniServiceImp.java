...
@Service
public class PahilocharanBillVuktaniServiceImp implements PahilocharanBillVuktaniService {
    ...
    public Object getAll(Long applicationNo) {
        message.list = da.getAll("from PahilocharanBillVuktani where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "47"));
        message.map.put("history", message.getHistory(applicationNo, "47"));
        return message.map;
    }
    ...
    public Object save(PahilocharanBillVuktani obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "47", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 47, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "47", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "47");
    }
}