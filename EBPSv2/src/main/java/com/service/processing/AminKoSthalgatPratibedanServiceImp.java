...
import com.dao.processing.DaoAminKoSthalgatPratibedan;
import com.model.application.ApplicationApproved;
import com.model.processing.AminKoSthalgatPratibedan;

@Service
public class AminKoSthalgatPratibedanServiceImp implements AminKoSthalgatPratibedanService {

    @Autowired
    DaoAminKoSthalgatPratibedan da;
    ...
    public Object getAll(String Authorization, long id) {
        message.list = da.getAll("from AminKoSthalgatPratibedan where applicationNo=" + id);
        ...
        message.map.put("comment", message.getComment("" + id, "57"));
        message.map.put("history", message.getHistory(id, "57"));
        return message.map;
    ...
    public Object save(AminKoSthalgatPratibedan obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "57", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 57, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "57", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
          return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "57");
    }
}