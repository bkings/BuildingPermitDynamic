...
@Service
public class SuperstructurekoNirmanKaryaServiceImp implements SuperstructurekoNirmanKaryaService {
    ...
    public Object getAll(Long applicationNo) {
        message.list = da.getAll("from SuperstructurekoNirmanKarya where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "52"));
        message.map.put("history", message.getHistory(applicationNo, "52"));
        return message.map;
    }
    ...
    public Object save(SuperstructurekoNirmanKarya obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "52", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 52, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "52", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "52");
    }
}
