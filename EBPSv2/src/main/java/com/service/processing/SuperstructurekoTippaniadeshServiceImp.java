...
@Service
public class SuperstructurekoTippaniadeshServiceImp implements SuperstructurekoTippaniadeshService {
    ...
    public Object getAll(long applicationNo) {
        message.list = da.getAll("from SuperstructurekoTippaniadesh where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "30"));
        message.map.put("history", message.getHistory(applicationNo, "30"));
        return message.map;
    }
    ...
    public Object save(SuperstructurekoTippaniadesh obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "30", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {
                message.db.save(message.getEnterByStatus(userType, 30, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "30", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "30");
    }
}
