...
@Service
public class MyadThapApprovalServiceImp implements MyadThapApprovalService {
    ...
    public Object getAll(long applicationNo) {
        message.list = da.getAll("from MyadThapApproval where applicationNo=" + applicationNo);
        ...
        message.map.put("comment", message.getComment("" + applicationNo, "53"));
        message.map.put("history", message.getHistory(applicationNo, "53"));
        return message.map;
    }
    ...
    public Object save(MyadThapApproval obj, String Authorization) {
        ...
        if (!message.checkSaveStatus(userType, "53", applicationNo)) {
        ...
        try {
            row = da.save(obj);
            if (row == 1) {             
                message.db.save(message.getEnterByStatus(userType, 53, applicationNo));
                message.setHistory(applicationNo, td.getUserType(), "53", "S", td.getUserName());
    ...
    public Object doApprove(Long applicationNo, ApplicationApproved obj, String Authorization) {
        return new ApplicationApprove().doApprove(applicationNo, obj, Authorization, "53");
    }
}