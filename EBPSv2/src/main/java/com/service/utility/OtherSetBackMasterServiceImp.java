...
import com.dao.utility.DaoOtherSetBackMaster;
import com.model.utility.OtherSetBackMaster;

import model.Message;

@Service
public class OtherSetBackMasterServiceImp implements OtherSetBackMasterService{
    @Autowired
    DaoOtherSetBackMaster da;
    ...
    public Object getAll() {
        return da.getAll("from OtherSetBackMaster order by id");
    }
    ...
    public Object save(OtherSetBackMaster obj, String Authorization) {
        ...
        try {
            try {
                sql = "SELECT coalesce(MAX(ID),0)+1 AS id FROM other_set_back_master";
    ...
    public Object delete(String id, String Authorization) {
        ...
        id = "'" + id.replace(",", "','") + "'";
        sql = "DELETE FROM other_set_back_master WHERE ID IN (" + id + ")";
        row = da.delete(sql);
        msg = da.getMsg();
        if (row > 0) {
            return message.respondWithMessage("Success");
