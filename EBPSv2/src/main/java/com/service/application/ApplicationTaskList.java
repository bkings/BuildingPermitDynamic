...
public class ApplicationTaskList {
	...
	public Object get(Long applicationNo, String constructionType, String year, String nibedakName, String kittaNo, String wardNo, String applicationStatus,
		 ...
		 * + enterBy + " forward_to_" + userType +
		 * "='I' AND coalesce(notice15_till_date,'" + DateConvert.today() + "')<='" +
		 * DateConvert.today() + "' AND ID=COALESCE(" + applicationNo + ",ID) " +
...
//                System.out.println("yourForm "+yourForm);
			...
			sql = "SELECT user_type \"userType\",rejected_by_user \"status\" FROM application_status WHERE application_no=" + Long.parseLong(message.map.get("applicantNo").toString());
			try {
				message.map.put("rejectedBy", message.db.getRecord(sql));
			} catch (Exception e) {
				e.printStackTrace();
				message.map.put("rejectedBy", "");
			}
