...
import com.service.processing.AminkoPratibedanService;

@RestController
@CrossOrigin
@RequestMapping("api/Processing/AminkoPratibedan")
public class AminkoPratibedanRestController {

    @Autowired
    AminkoPratibedanService service;

    @GetMapping("/{applicationNo}")
    public Object show(@PathVariable Long applicationNo, @RequestHeader(value = "Authorization") String Authorization) {
        return service.getAll(applicationNo, Authorization);
    }
    ...
    public Object doSave(@PathVariable Long applicationNo, @RequestBody String jsonData, @RequestHeader(value = "Authorization") String Authorization) {
        AminkoPratibedan obj = new AminkoPratibedan();
