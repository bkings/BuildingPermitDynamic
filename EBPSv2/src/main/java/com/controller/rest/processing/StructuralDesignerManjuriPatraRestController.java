...
import com.service.processing.StructuralDesignerManjuriPatraService;

@RestController
@CrossOrigin
@RequestMapping("api/Processing/Phase1/StructuralDesignerManjuriPatra")
public class StructuralDesignerManjuriPatraRestController {

	@Autowired
	StructuralDesignerManjuriPatraService service;

	@GetMapping("/{applicationNo}")
	public Object index(@PathVariable String applicationNo,
			@RequestHeader(value = "Authorization") String Authorization) {
		return service.getAll(Authorization, applicationNo);
	}

	@PostMapping("/{applicationNo}")
	public Object doSave(@Valid @RequestBody StructuralDesignerManjuriPatra obj, @PathVariable Long applicationNo,
			@RequestHeader(value = "Authorization") String Authorization) {
