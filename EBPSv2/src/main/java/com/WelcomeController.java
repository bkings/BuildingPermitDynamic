...
//        map.put("token", "Bearer " + token);
    ...
    public Object doLoginget(HttpServletRequest request, @RequestParam String loginId, @RequestParam String password) {
        ...
        sql = "SELECT LOGIN_ID \"userCode\",USER_NAME \"userName\",USER_TYPE \"userType\",STATUS \"status\",coalesce(DB_PASSWORD,'') \"dbPassword\"  FROM organization_user WHERE (LOGIN_ID='" + loginId.toUpperCase() + "' OR email='" + loginId + "')";
        ...
        try {
            ...
            if (!status.equals("Y")) {
                return message.respondWithError("Your Login Lock");
            }
            ...
            if (userType.equalsIgnoreCase("D")) {
                ...
                if (m.get("designer_login_type").toString().equalsIgnoreCase("public")) {
                    sql = "SELECT coalesce(user_name,'') \"userName\",coalesce(signature,'') signature,coalesce(stamp,'') stamp,coalesce(photo,'') photo,coalesce(municipal_reg_no,'') \"municipalRegNo\",coalesce(license_no,'') \"licenseNo\",coalesce(education_qualification,''),coalesce(address,'') address,coalesce(consultancy_name,'कृपया कन्सल्टेन्सिको नाम प्रविष्ट गर्नुहोस् ।') \"consultancyName\" FROM organization_user  WHERE login_id='" + userCode + "' ";
                } else {
                    sql = "SELECT coalesce(user_name,'') \"userName\",coalesce(signature,'') signature,coalesce(stamp,'') stamp,coalesce(photo,'') photo,coalesce(municipal_reg_no,'') \"municipalRegNo\",coalesce(license_no,'') \"licenseNo\",coalesce(education_qualification,''),coalesce(address,'') address,coalesce(consultancy_name,'कृपया कन्सल्टेन्सिको नाम प्रविष्ट गर्नुहोस् ।') \"consultancyName\" FROM organization_user  WHERE login_id='" + userCode + "' AND valid_date>='" + DateConvert.today() + "'";
                }
...
//            sql = "SELECT id ,amin_approval \"aminApproval\",chief_approval \"chiefApproval\",designer_approval \"designerApproval\",engr_approval \"engrApproval\",sub_engr_approval \"subEngrApproval\" ,enter_by \"enterBy\",\"name\",rajasow_approval \"rajasowApproval\",\"table_name\" AS \"tableName\",view_url \"viewUrl\",COALESCE(poste_approval,'N') \"posteApproval\",COALESCE(postf_approval,'N') \"postfApproval\",COALESCE(postg_approval,'N') \"postgApproval\" FROM form_name_master";
            ...
            sql = "SELECT forward_by \"forwardBy\",forward_to \"forwardTo\",form_group \"formGroup\" FROM application_forwarding_setup WHERE forward_by='" + userType + "'";
            message.map.put("applicationForwardingSetup", db.getRecord(sql));
            sql = "SELECT forward_by \"forwardBy\",forward_to \"forwardTo\",form_group \"formGroup\",skip_parameter \"skipParameter\" FROM application_forwarding_setup WHERE forward_by='" + userType + "'";
            message.map.put("applicationForwardingSetup", db.getRecord(sql));
            ...
            for (int i = 0; i < list.size(); i++) {
                m = (Map) list.get(i);
                userTypeAll += "{\"userType\":\"" + m.get("userType") + "\",\"designation\":\"" + m.get("designation") + "\",\"column\":\"" + m.get("column") + "\",\"actionStatus\":\"" + m.get("actionStatus") + "\"}";
            }
    ...
    @ResponseBody
    public Object changePassword(@RequestHeader(value = "Authorization") String Authorization, @RequestParam String password, @RequestParam String oldPassword) {
        model.Message message = new model.Message();
        ...
        sql = "UPDATE organization_user SET db_password='" + password + "' WHERE login_id='" + td.getUserId() + "'";
        int row = db.save(sql);
        if (row == 0) {
...
//        String password = String.valueOf(new Random().nextDouble()).substring(3, 9);
        ...
        sql = "UPDATE organization_user set db_password='" + password + "'where login_id='" + id + "'";
        message.db.save(sql);
    ...
    public String logout() {
        ...
        for (int i = 0; i < data.length; i++) {
            sql = "INSERT INTO form_name_master(id,page_code,\"name\",view_url) VALUES('" + data[i][0] + "','" + data[i][1] + "','" + data[i][2] + "','" + data[i][4] + "');";
            db.save(sql);
        }
        return "OK";
    }
